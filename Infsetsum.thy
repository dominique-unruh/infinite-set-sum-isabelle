...
proof -
  ...
  have \<mu>sum: "sum (\<lambda>x. norm (\<mu> x)) F \<le> B" if "finite F" and "F \<subseteq> T" for F
    unfolding B_def using assms that apply (auto simp flip: infsetsum_finite)
    apply (rule infsetsum_mono_neutral_left) by auto
...
proof (rule abs_summable_finite_sumsI)
  ...
  also have "\<dots> \<le> (\<Sum>\<^sub>ai\<in>A. norm (x i * x i)) + (\<Sum>\<^sub>ai\<in>A. norm (y i * y i))" 
    using aux a1
    by (smt (verit, del_insts) abs_summable_on_normI abs_summable_on_subset assms(2) b4 infsetsum_cong infsetsum_mono_neutral_left norm_ge_zero)
  also have "\<dots> = B"
...
text \<open>The following lemma is a strengthening of @{thm [source] infsetsum_Sigma}
  from \<^theory>\<open>HOL-Analysis.Infinite_Set_Sum\<close>. It does not require \<^term>\<open>A\<close> and \<^term>\<open>B\<close> to be countable.\<close>
...
proof (rule abs_summable_finite_sumsI)
  ...
  have sum_FS'_B: "(\<Sum>x\<in>F\<inter>S' i. norm (f x)) \<le> B i" for i
    unfolding B_def using finite_F apply (auto simp flip: infsetsum_finite)
    apply (rule infsetsum_mono_neutral_left) 
    using S'_S f_sum_S' FT assms by auto
...
text \<open>The following lemma is a strengthening of @{thm [source] infsetsum_prod_PiE} from \<^theory>\<open>HOL-Analysis.Infinite_Set_Sum\<close>.
  It removes the premise that \<^term>\<open>B\<close> is countable.\<close>
...
text \<open>The following theorem is a slight variant of @{thm [source] infsetsum_cmult_left}.
  It avoids the premise of @{thm [source] infsetsum_cmult_left}, but additionally
  requires a division ring (type class \<^class>\<open>division_ring\<close>).\<close>
...
lemma abs_summable_on_zero_diff:
  ...
  by (smt (z3) Un_Diff_cancel Un_subset_iff abs_summable_on_0 abs_summable_on_comparison_test
        abs_summable_on_subset abs_summable_on_union assms(1) assms(2) norm_zero order_refl)

text \<open>This lemma logically belongs to the section about real numbers.
  However, it is used in the proof of \<open>abs_summable_on_Sigma_iff\<close> so we cannot move it there.\<close>

lemma infsetsum_0D_real:
...
lemma
  ...
  shows   abs_summable_times_product: "(\<lambda>(x,y). f x * g y) abs_summable_on A \<times> B"
    and   infsetsum_times_product: "infsetsum (\<lambda>(x,y). f x * g y) (A \<times> B) =
...
text \<open>This lemma is almost identical to the existing @{thm [source] infsetsum_mono_neutral}
  from \<^theory>\<open>HOL-Analysis.Infinite_Set_Sum\<close> except that the premise
  \<^prop>\<open>\<And>x. x \<in> A \<Longrightarrow> f x \<le> g x\<close> is replaced by the weaker \<^prop>\<open>\<And>x. x \<in> A \<inter> B \<Longrightarrow> f x \<le> g x\<close>.
  (The fact that the original lemma has \<^term>\<open>A\<close> instead of \<^term>\<open>A\<inter>B\<close> might be just a mistake.
  The original proof goes through unchanged.)\<close>
lemma infsetsum_mono_neutral:
  fixes f g :: "'a \<Rightarrow> real"
  assumes "f abs_summable_on A" and "g abs_summable_on B"
  assumes "\<And>x. x \<in> A \<inter> B \<Longrightarrow> f x \<le> g x"
...
text \<open>Logically belongs here but is proven in the previous section:\<close>
lemmas infsetsum_0D_real
text \<open>@{thm infsetsum_0D_real[no_vars]}\<close>
...
lemma infsetsum_mono_neutral_complex:
  fixes f :: "'a \<Rightarrow> complex"
  assumes [simp]: "f abs_summable_on A"
    and [simp]: "g abs_summable_on B"
...
lemma infsetsum_mono_complex:
  ...
  assumes "f abs_summable_on A" and g_sum: "g abs_summable_on A"
  assumes "\<And>x. x \<in> A \<Longrightarrow> f x \<le> g x"
  shows   "infsetsum f A \<le> infsetsum g A"
  apply (rule infsetsum_mono_neutral_complex)
  using assms by auto
...
text \<open>Complex variant of @{thm [source] infsetsum_nonneg}:\<close>
lemma infsetsum_nonneg_complex:
  fixes f :: "'a \<Rightarrow> complex"
...
lemma infsetsum_cmod:
  ...
  by (smt (verit, best) Re_infsetsum assms(1) cmod_eq_Re fnn infsetsum_cong infsetsum_nonneg_complex less_eq_complex_def zero_complex.sel(1) zero_complex.sel(2))
